          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                ...
                把购买的产品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
  ...
  methods: {
    ...
    //  2:需要知道这次请求成功还是失败，如果成功进行路由跳转，如果失败，需要给用户提示
     try {
      ...
      // 下面这种手段路由跳转一级传递参数是可以使用的
      // 一些简单的数据skuNum，通过query形式给路由组件传递过去
      // 产品信息的数据【比较复杂：skuInfo】，通过会话存储(sessionStorage)[不持久化，会话结束数据再消失]
      // 本地存储|会话存储，一般存储的是字符串
